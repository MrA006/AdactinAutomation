...
test('booking with expired credit card', async ({ page }) => {
    test.slow();
    ...
    await page.selectOption('#cc_exp_year', '2011');
    await page.fill('#cc_cvv', '093');

    page.on('dialog', async dialog => {
        console.log('Alert message detected:', dialog.message());
        
        expect(dialog.message()).toBe('Expiry year cannot be in Past');
